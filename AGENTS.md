...
1. **采用最小修改原则**：优先进行精确局部替换，不无故重构或改写不相关文件。
2. **严格保持 TypeScript 类型安全**：新增接口或模型时同步更新 `server/db/schema.ts` 与 `src/lib/types.ts`。
3. **改动验证**：修改前端代码后运行 `bun run build` 确保构建无错；修改后端接口后使用 `bun -e` 快速发起验证请求。
