      ...
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 w-[92%] max-w-2xl bg-white dark:bg-stone-900 rounded-2xl border border-stone-200/90 dark:border-stone-800 shadow-modal p-2.5 sm:p-3 flex items-center justify-between gap-2.5 animate-in slide-in-from-bottom-5 duration-200">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          <div className="relative">
            ...
            {topicDropdownOpen && (
              <div className="absolute bottom-full mb-2 left-0 w-48 rounded-xl border border-stone-200 dark:border-stone-800 bg-white dark:bg-stone-900 p-1 shadow-card z-50">
                <div className="p-1.5 text-[11px] font-bold text-stone-400">选择要关联的选题</div>
                {topicsData?.topics.map((t) => (
                  <button
                    key={t.id}
                    type="button"
                    onClick={() => {
                      onBatchAction('add_topic', { topicId: t.id });
